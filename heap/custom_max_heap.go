...
type CustomMaxHeap[K Ordered] struct {
	array []*K
}
...
func (h *CustomMaxHeap[K]) CustomMaxHeapifyUp(idx int) {
	...
	for (*h.array[idx]).Greater(*h.array[parent_idx]) {
...
func (h *CustomMaxHeap[K]) CustomMaxHeapifyDown(idx int) {
	if right(idx) < len(h.array) && ((*h.array[right(idx)]).Greater(*h.array[idx]) || (*h.array[left(idx)]).Greater(*h.array[idx])) {
		if (*h.array[right(idx)]).Greater(*h.array[left(idx)]) {
	...
	} else if left(idx) < len(h.array) && ((*h.array[left(idx)]).Greater(*h.array[idx])) {
		h.swap(left(idx), idx)
		h.CustomMaxHeapifyDown(left(idx))
	}
}
