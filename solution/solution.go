...
func predictTheWinner(nums []int) bool {
	max_win_by := make(map[int]map[int]int)
	for i:=0; i<len(nums); i++ {
		max_win_by[i] = make(map[int]int)
	}
    return topDownMaxWinBy(0, len(nums)-1, nums, max_win_by) >= 0
}

/*
Top-down helper method for the above problem
*/
func topDownMaxWinBy(start int, end int, nums []int, max_win_by map[int]map[int]int) int {
	_, ok := max_win_by[start][end]
	if !ok {
		// Need to solve this problem
		if start == end {
			// Base case
			max_win_by[start][end] = nums[start]
		} else {
			// Pick the first or last and see which result is better
			max_win_by[start][end] = max(
										nums[start] - topDownMaxWinBy(start+1, end, nums, max_win_by), 
										nums[end] - topDownMaxWinBy(start, end-1, nums, max_win_by),
									)
		}
	}
	return max_win_by[start][end]
}
